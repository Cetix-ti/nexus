...
export default function PortalLoginPage() {
  ...
  useEffect(() => {
    fetch("/api/auth/providers", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((data) => {
        setMsAvailable(!!data?.["microsoft-entra-id"]);
        setGoogleAvailable(!!data?.["google"]);
      })
      .catch(() => { setMsAvailable(false); setGoogleAvailable(false); });
  }, []);
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm p-7">
          ...
          {msAvailable === false && (
            <p className="mt-2 text-[11px] text-neutral-400 text-center">
              Microsoft Entra ID non configuré
            </p>
          )}
          ...
          {googleAvailable !== false && (
            <button
              ...
              className="mt-2 flex w-full items-center justify-center gap-3 rounded-lg border border-neutral-300 bg-white px-4 py-2.5 text-sm font-semibold text-neutral-800 hover:bg-neutral-50 hover:border-neutral-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
